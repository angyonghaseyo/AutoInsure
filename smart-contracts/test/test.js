...
describe("Insurer Contract", function () { 
    ...
    before(async function () {
        ...
        insurer = await Insurer.connect(owner).deploy();
    ...
    it("should only allow the company (owner) to add policies", async () => {
        await expect(insurer.connect(player1).addPolicy(1, 5, 10, 2, 30))
            .to.be.revertedWith("Insurer, Only the owner can call this function");

        await expect(insurer.connect(owner).addPolicy(1, 5, 10, 2, 30))
            .to.emit(insurer, "PolicyAdded").withArgs(1);
    });
    ...
    it("should allow users to buy a policy", async () => {
        ...
        await expect(
            insurer.connect(player1).buyPolicy(1, "SQ001", currentTimestamp + 86400, { value: ethers.parseEther("1") })
        ).to.emit(insurer, "PolicyBought").withArgs(1);

        await expect(
            insurer.connect(player2).buyPolicy(1, "SQ002", currentTimestamp + 172800, { value: ethers.parseEther("1") })
        ).to.emit(insurer, "PolicyBought").withArgs(1);
    });
    ...
    it("should allow a policyholder to claim a policy", async () => {
        ...
        // Claim policy
        await expect(insurer.connect(player1).claimPolicy())
            .to.emit(insurer, "Payout")
            .withArgs(payoutAmount, player1.address);
    ...
    it("should allow only the owner to delete a policy", async () => {
        await expect(insurer.connect(player1).deletePolicy(1))
            .to.be.revertedWith("Insurer, Only the owner can call this function");

        await expect(insurer.connect(owner).deletePolicy(1))
            .to.emit(insurer, "PolicyDeleted").withArgs(1);
    });

    it("should allow the owner to withdraw funds", async () => {
        const initialOwnerBalance = await ethers.provider.getBalance(owner.address);

        await expect(insurer.connect(player1).withdrawFunds(10))
            .to.be.revertedWith("Insurer, Only the owner can call this function");

        await expect(insurer.connect(owner).withdrawFunds(10))
            .to.changeEtherBalance(owner, ethers.parseEther("10"));
