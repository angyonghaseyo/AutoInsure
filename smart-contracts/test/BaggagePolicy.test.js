...
describe("BaggagePolicy", function () {
  ...
  describe("Policy purchase", function () {
    ...
    it("should reject purchase of deactivated policies", async function () {
      await expect(
        baggagePolicy.connect(user1).purchasePolicy(
          deactivatedTemplate,
          "Premium leather suitcase and carry-on bag",
          user1.address,
          { value: deactivatedTemplate.premium }
        )
    ...
    it("should allow purchase of active policy", async function () {
      const tx = await baggagePolicy.connect(user1).purchasePolicy(
        activeTemplate,
        "Premium leather suitcase and carry-on bag",
        user1.address,
        { value: activeTemplate.premium }
      );
      
      await tx.wait();
      
      const policies = await baggagePolicy.getUserPolicies(user1.address);
    ...
    it("should increment policy ID for each purchase", async function () {
      const firstPolicies = await baggagePolicy.getUserPolicies(user1.address);
      const firstPolicyId = firstPolicies[0].policyId;
      
      // Purchase another policy
      await baggagePolicy.connect(user1).purchasePolicy(
        activeTemplate,
        "Vintage camera equipment and accessories",
        user1.address,
        { value: activeTemplate.premium }
      );
      
      const updatedPolicies = await baggagePolicy.getUserPolicies(user1.address);
    ...
    it("should allow different users to purchase policies", async function () {
      await baggagePolicy.connect(user2).purchasePolicy(
        activeTemplate,
        "Surfboard and diving equipment",
        user2.address,
        { value: activeTemplate.premium }
      );
      
      const user2Policies = await baggagePolicy.getUserPolicies(user2.address);
  ...
  describe("Policy retrieval", function () {
    ...
    it("should get all policies for a given user", async function () {
      const user1Policies = await baggagePolicy.getUserPolicies(user1.address);
    ...
    it("should get all policies by template ID", async function () {
      const policiesByTemplate = await baggagePolicy.getUserPoliciesByTemplate(activeTemplate.templateId);
    ...
    it("should return empty array for non-existent users", async function () {
      ...
      const policies = await baggagePolicy.getUserPolicies(nonExistentUser);
      expect(policies.length).to.equal(0);
    });

    // 11. Insurer-only function check
    it("should allow only insurer to get all policies", async function () {
      // Insurer should be able to get all policies
      const allPolicies = await baggagePolicy.connect(insurer).getAllPolicies();
      expect(allPolicies.length).to.equal(3); // Total policies across all users
      
      // Non-insurer should not be allowed
      await expect(
        baggagePolicy.connect(user1).getAllPolicies()
      ).to.be.revertedWith("BaggagePolicy: Only the insurer can call this function");
    });
  });
  ...
  describe("Policy expiration", function () {
    ...
    it("should not allow non-insurer to mark policy as expired", async function () {
      await expect(
        baggagePolicy.connect(user1).markPolicyAsExpired(0)
      ).to.be.revertedWith("BaggagePolicy: Only the insurer can call this function");
    });
    ...
    it("should not allow marking a non-existent policy as expired", async function () {
      ...
      await expect(
        baggagePolicy.connect(insurer).markPolicyAsExpired(nonExistentPolicyId)
      ).to.be.revertedWith("Invalid policyId");
    });
    ...
    it("should not mark a policy as expired before its expiry date", async function () {
      ...
      await expect(
        baggagePolicy.connect(insurer).markPolicyAsExpired(0)
      ).to.be.revertedWith("Policy has not expired yet");
    });
    ...
    it("should mark a policy as expired after time manipulation", async function () {
      ...
      const advanceTime = BigInt(policy.template.coverageDurationDays) * 24n * 60n * 60n + 1n;
