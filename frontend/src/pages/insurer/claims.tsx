...
const ClaimPage: React.FC = () => {
  ...
  useEffect(() => {
    const fetchPolicies = async () => {
      ...
      try {
        ...
        const claimablePolicies = policyDetails.filter((p) => p.status === PolicyStatus.Active && !p.isPaid && !p.isClaimed);
  ...
  return (
    <div className="max-w-6xl mx-auto p-6">
      <Title level={2}>Policy Claims</Title>
      ...
      {!account ? (
        ...
        <Alert message="No Claimable Policies" description="There is no active policies eligible for claims." type="info" showIcon style={{ marginBottom: "20px" }} />
      ) : (
        <>
          <Paragraph>Select a policy to check its claim status.</Paragraph>
          <Select placeholder="Select a policy" style={{ width: "100%", marginBottom: "20px" }} onChange={handlePolicySelect}>
            {policies.map((policy) => (
